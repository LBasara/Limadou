...
int n_tot_ev = 0; //number of events per step, default value. I assign it later in the code
...
void cal_Silicons(string name_root_file, string typeofparticle){
	...
	const int n_tot_ev = tree->GetEntries();
	...
	int * ADC2[n_tot_ev]; // 1000 pointers to the ADC array !
	...
	float * comnoise_real[n_tot_ev]; // 1000 pointers to the comnoise array  !
	...
	for(int i=0; i<n_chann; ++i) { ADC[i] = new int[n_tot_ev];} // initialize ADC one way
	for(int j=0; j<n_tot_ev; j++){ADC2[j] = new int[n_chann];} // initialize ADC other way
	for (int j=0; j<n_tot_ev; j++){ comnoise_real[j] = new float[n_VA];} // initialize comnoise_real one way
	for (int i=0; i<n_VA; i++){ comnoise_real2[i] = new float[n_tot_ev];} // initialize comnoise_real other way
	...
	for (int s=0; s<n_steps;++s){
		clock_t start = clock();
		for (int j=0; j<n_tot_ev; j++){ 		// event index
			tree->GetEntry(s*n_tot_ev+j);				// all channels for a fixed event
		...
		for (int j=0; j<n_tot_ev; j++){			// loop over the events
	...
	for (int i=0; i<n_chann; i++){
		...
		non_gauss_index[i]=(non_gauss[i]-0.0027*n_tot_ev)/(sqrt(0.0027*n_tot_ev));
	}
	// nice output!!
	std::ofstream calib;
	string calib_name;
